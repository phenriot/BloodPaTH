...
* *dist_risk* : parameters of the distributions of the of risk of getting infected for each of the procedures. The first three columns correspond to the values of the parameters of these distributions and the fourth to the type of distribution (log-normal : "lnorm", gaussian :"norm" or PERT : "pert"). Only the two first columns are used for the [log-normal](https://en.wikipedia.org/wiki/Log-normal_distribution) and [normal](https://en.wikipedia.org/wiki/Normal_distribution) (for $\mu$ and  	$\sigma^2$) distributions; all columns are used for the [PERT](https://en.wikipedia.org/wiki/PERT_distribution) distribution (for $a$, $b$ and $c$). The last row will be a "fake" row for the "no procedure" event (example : the last row can be added as follows &rarr; `rbind(your_dist_risk_data_frame,c(0,0,NA,"lnorm"))`
