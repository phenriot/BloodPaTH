...
* ***table_procedures_devices*** : correspondence table between procedures (first column) and devices (second column). Its size depend on the number of associations between procedures and devices (**example** : if you have 2 types of procedures and 3 types of devices, if each type of device is used during each type of procedure, the table will have 2*3=6 rows and will be &rarr; `matrix(data = c(1,1,1,2,1,3,2,1,2,2,2,3), ncol = 2,byrow = T,dimnames = list(c(),c('ID procedure','ID device')))` ) | <ins> type = integer matrix </ins>
...
* **incidence** : gives the incidence at each time-step at a setting level for the baseline scenario. | <ins> type = integer vector </ins>
