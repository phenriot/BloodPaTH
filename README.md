...
* *WT_matrix* : merged transition matrices for all admission routes; the *list_to_combined_matrices* R function helps to convert a list of transition matrices as follows &rarr; $${\color{green}list{_}to{_}combined{_}matrices(}$$ input = *your_list*, type = *1*, nb_wards = *nb_wards*$${\color{green})}$$
